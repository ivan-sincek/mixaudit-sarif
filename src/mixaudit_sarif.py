...
def basic():
	global proceed
	proceed = False
	print("MixAudit SARIF v1.5 ( github.com/ivan-sincek/mixaudit-sarif )")
	print("")
...
def find_line(identifier, file, package):
	position = count = 0
	# NOTE: String to match in the file.
	...
	stream.close()
	if not position:
		print_error(("{0}: Cannot find the string '{2}' in '{1}'").format(identifier, file, package))
	return position
...
def cve_details(identifier, cve):
	...
	try:
		response = json.loads(requests.get(("https://services.nvd.nist.gov/rest/json/cve/1.0/{0}").format(cve)).text)
		tmp["security-severity"] = response["result"]["CVE_Items"][0]["impact"]["baseMetricV3"]["cvssV3"]["baseScore"]
		tmp["precision"] = response["result"]["CVE_Items"][0]["impact"]["baseMetricV3"]["cvssV3"]["baseSeverity"].lower()
		tmp["tags"].extend([cve, tmp["precision"]])
	except Exception:
		tmp = {}
		print_error(("{0}: Cannot fetch the {1} details").format(identifier, cve))
	return tmp
...
def append_slash(string):
	const = "/"
	if not string or string[-1] != const:
		string += const
	return string

def format(results, out, directory):
	sarif = sarif_schema()
	try:
	...
	write_file(json.dumps(sarif, indent = 4), out)

if proceed:
	print("#######################################################################")
	print("#                                                                     #")
	print("#                         MixAudit SARIF v1.5                         #")
	...
	if results:
		format(results, args["out"], append_slash(args["directory"]))
